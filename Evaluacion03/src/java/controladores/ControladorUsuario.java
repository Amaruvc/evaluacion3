...
public class ControladorUsuario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        switch(accion){
            case "1": iniciarSesion(request,response);
                break;
            default: response.sendRedirect("login.jsp?msj=Acción no permitida");
        }
        }else{
            response.sendRedirect("login.jsp?msj=Acción no permitida");
        }
    }
    ...
    private void iniciarSesion(HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        if(temporal != null){
            ...
            response.sendRedirect("login.jsp?msj=Run inexistente");
        }
        }catch(Exception e){
